...
EnBatch interface reference [size]:
...
--]]
local EnBatch = torch.class('EnBatch')
...
--]]
function EnBatch:__init(src, srcFeatures, tgt, tgtFeatures)
  src = src or {}
  ...
  if tgt ~= nil then
    -- don't ignore EOS token in targetInput; this invalidates nonzeros, but that's fine
  ...
  for b = 1, self.size do
    ...
    if tgt ~= nil then
      ...
      local targetInput = tgt[b]:narrow(1, 1, targetLength) -- should include EOS
...
--]]
function EnBatch:setSourceInput(sourceInput)
...
--]]
function EnBatch:setTargetInput(targetInput)
...
--]]
function EnBatch:setTargetOutput(targetOutput)
...
function EnBatch:getSourceInput(t)
...
function EnBatch:getTargetInput(t)
  -- If a regular input, return word id, otherwise a table with features.
...
function EnBatch:getTargetOutput(t)
  assert(false)
  -- If a regular input, return word id, otherwise a table with features.
